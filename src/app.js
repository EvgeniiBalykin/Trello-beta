...
document.addEventListener("click", (event) => {
  ...
  if (event.target.classList.contains("popup-edit-button__edit")) {
    localStorage.setItem(cardId, JSON.stringify(editCard()));
    editPopup.style.display = "none";
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      if (cardId === id) {
        ...
        const timeContainer = cards[i].querySelector(".card-user__time");
        const hours = JSON.parse(localStorage.getItem(cardId)).hours;
        const minutes = JSON.parse(localStorage.getItem(cardId)).minutes;
  ...
  if (event.target.classList.contains("card-description__move-button")) {
    ...
    if (cardData.container === "progress") {
      const confirmToDone = confirm(
        "Do you really want to move this card to 'Done' column?"
      );
      ...
      if (inprogressCountEl.innerText < 6) {
        const confirmToInProgress = confirm(
          "Do you really want to move this card to 'In Progress' column?"
        );
...
function editCard() {
  return {
    ...
    minutes: formatMinutes(new Date().getMinutes()),
  };
}
